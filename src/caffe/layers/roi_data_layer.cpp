...
RoIDataLayer<Dtype>::RoIDataLayer(const LayerParameter& param)
  : BasePrefetchingRoIDataLayer<Dtype>(param),
    reader_(param) {
}
...
template <typename Dtype>
void RoIDataLayer<Dtype>::DataLayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  const int batch_size = this->layer_param_.data_param().batch_size();
  const RoIDataParameter& roi_data_param =
      this->layer_param_.roi_data_param();
  for (int i = 0; i < roi_data_param.batch_sampler_size(); ++i) {
    batch_samplers_.push_back(roi_data_param.batch_sampler(i));
  }
  label_map_file_ = roi_data_param.label_map_file();
  ...
  vector<int> top_shape =
      this->data_transformer_->InferBlobShape(roi_datum.datum());
  ...
  // For each ROI R = [batch_index x1 y1 x2 y2]: max pool over R
    ...
    roi_shape[0]= max_roi_per_ * batch_size;
    roi_shape[1]= 5;
  ...
  if (this->output_labels_) {
    ...
    num_class_=roi_data_param.get_num_class();
...
void RoIDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  // Use data_transformer to infer the expected blob shape from roi_datum.
  vector<int> top_shape =
      this->data_transformer_->InferBlobShape(roi_datum.datum());
  this->transformed_data_.Reshape(top_shape);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (transform_param.has_distort_param()) {
      ...
      this->data_transformer_->DistortImage(roi_datum.datum(),
